    ...
    "bilel@example.com": {
        ...
        "profile": UserProfile(
            ...
            history=[
                TenderWin(
                    ao_id="SEAO-001",
                    title="Implantation Odoo pour PME",
                    specialty="ERP Odoo",
                    date_awarded="2023-05-10",
                ),
                TenderWin(
                    ao_id="SEAO-002",
                    title="Modernisation ServiceNow ITSM",
                    specialty="ITSM / ServiceNow",
                    date_awarded="2024-02-18",
                ),
                TenderWin(
                    ao_id="SEAO-003",
                    title="Plateforme AO Collector POC",
                    specialty="Data & IA AO",
                    date_awarded="2024-11-01",
                ),
            ],
        ),
    },
    "cloud.consultant@example.com": {
        ...
        "profile": UserProfile(
            ...
            history=[
                TenderWin(
                    ao_id="SEAO-010",
                    title="Migration Oracle vers cloud hybride",
                    specialty="Cloud hybride & sécurité",
                    date_awarded="2022-09-30",
                ),
                TenderWin(
                    ao_id="SEAO-011",
                    title="Centre de services infonuagique",
                    specialty="Cloud hybride & sécurité",
    ...
    "odoo.partner@example.com": {
        ...
        "profile": UserProfile(
            ...
            history=[
                TenderWin(
                    ao_id="SEAO-020",
                    title="Implantation Odoo finance & RH",
                    specialty="ERP Odoo finance & RH",
                    date_awarded="2021-06-15",
                ),
                TenderWin(
                    ao_id="SEAO-021",
                    title="Odoo pour la gestion de projets publics",
                    specialty="ERP Odoo finance & RH",
...
# ------------------------------------------------------------------
# Dépendances FastAPI pour récupérer le user courant
# ------------------------------------------------------------------
...
def get_current_user(authorization: Optional[str] = Header(default=None)) -> AuthenticatedUser:
    """
    Dépendance obligatoire. Soulève 401 si le token est invalide.
    """
...
def get_current_user_optional(
    ...
    """
    Version *optionnelle* : renvoie None si pas de token ou token invalide.
    Utile pour les routes /tools.
    """
    if not authorization:
        return None
    try:
